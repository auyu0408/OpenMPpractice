...
int getfile(FILE** files, char *dir_name)
{
    ...
    struct dirent *inside;//regular file temporary
    char filename[100];
    ...
    {
        if(inside->d_type == DT_REG)//if inside is a regular file, open with fopen
        {
...
void tokenize(char *data, char* keywords[], int total_word, int key_count[])
{
    ...
    while(line != NULL)
    {
        ...
        {
            if(strcmp(line, keywords[count]) == 0)//if data belongs to keywords, add count
            {
                key_count[count]++;
                break;
            }
...
void prod_con(int total_threads, FILE* file[], int file_count, char* keywords[], int word_count, int key_count[])
{
    ...
    #pragma omp parallel num_threads(total_threads)
    {
        ...
        if(my_id < producer)
        {
            ...
            #pragma omp atomic//producer done it's works
            producer_done++;
        }
        else
        {
...
{
    int thread_sz;//# of total thread
    FILE *file[MAX_FILES];//save file in words/
    int total_file = 0;//# of total words file
    int i;//counting variable
    
    int total_key = 0;//# of total keys
    char* keys[MAX_KEYS];//string array to save keywords
    ...
    total_file = getfile(&file, "words/");//open txt file
    
    prod_con(thread_sz, file, total_file, keys, total_key, keys_count);//produce and consume
